...
# Post-training INT7 weight-only quantization with true packing + cached fast path
...
def evolve_bitplanes(bitplanes, steps=8):  # ✅ changed from 16 -> 8
    evolved = []
...
_t = _tick()
N = len(X_imgs)
X_feat = np.empty((N, 8*14*14), dtype=np.float32)  # ✅ changed 16 -> 8
...
model = SoftmaxLogistic(8*14*14, 10).to(device)  # ✅ changed 16 -> 8
...
# -----------------------------
# 8) Post‐training INT7 weight-only quantization
#     with TRUE bit packing + cached dequant fast path
# -----------------------------
print("8) Quantizing weights to INT7 (per-channel, symmetric) with true bit packing…")
_t = _tick()

def _signed_range(bits: int):
    assert 2 <= bits <= 7, "Supported bits: 2..7"
    qmin = -(1 << (bits - 1))
    qmax =  (1 << (bits - 1)) - 1
    return qmin, qmax
...
class PackedIntNLinear(nn.Module):
    ...
    def __init__(self, float_linear: nn.Linear, bits: int, cache_dequant: bool = True):
        ...
        self.register_buffer("packed", packed)                    # [out, nbytes] uint8
        self.register_buffer("scales", scales.to(torch.float32))  # [out]
        ...
        # Lazy cache for dequantized matrix (not saved in state_dict)
    ...
    def _get_W_deq(self, device: torch.device) -> torch.Tensor:
        ...
        qW_int8 = unpack_intN_rows(self.packed, self.in_features, self.bits)  # [out,in] int8 on CPU
    ...
    def extra_repr(self) -> str:
        mem_int8_elems = self.out_features * self.in_features
        packed_bytes = int(self.packed.numel())
        ratio_vs_int8 = packed_bytes / max(1, mem_int8_elems)
        return (f"bits={self.bits}, in_features={self.in_features}, out_features={self.out_features}, "
                f"packed_bytes={packed_bytes}, approx_storage_ratio_vs_int8={ratio_vs_int8:.3f}, "
                f"cache_dequant={self.cache_dequant}")

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        device = x.device
        if self.bias is not None and self.bias.device != device:
            self.bias = self.bias.to(device)
        W_deq = self._get_W_deq(device)  # FP32 [out, in]
...
def quantize_model_to_packed_intN(model_fp32: nn.Module, bits: int, cache_dequant: bool = True) -> nn.Module:
    """
    Recursively convert nn.Linear -> PackedIntNLinear(bits). Leave other layers unchanged.
    """
...
# Make one model: INT7 only
qmodel7 = quantize_model_to_packed_intN(model, bits=7, cache_dequant=True).eval()
times["8_quantize_s"] = _tock(_t)
print("   ✅ Quantization + packing done for INT7.")
print(qmodel7)
print(f"   ⏱ Step 8 time: {times['8_quantize_s']:.3f} s\n")

# -----------------------------
# 9) Final evaluation on test set (INT7, CPU)
# -----------------------------
print("9) Evaluating INT7-packed model on CPU…")
Xte_cpu, yte_cpu = X_te_t.cpu(), y_te_t.cpu()

t7 = time.time()
with torch.no_grad():
    logits7 = qmodel7(Xte_cpu)   # first call unpacks+dequants and caches
    preds7  = logits7.argmax(1)
    acc7 = (preds7 == yte_cpu).float().mean().item() * 100
t7 = time.time() - t7

times["9_eval_cpu_s"] = t7
print(f"   Test Accuracy (INT7-packed): {acc7:.2f}%   ⏱ {t7:.3f}s\n")

# Use INT7 outputs for downstream plots
logits = logits7
preds  = preds7
...
plt.plot([0,1],[0,1],'k--')
plt.title("ROC Curves (INT7-packed)")
...
plt.title("Confusion Matrix (INT7-packed)")
plt.tight_layout()
plt.show()
