...
# Post-training INT6 weight-only quantization with true packing + cached fast path
...
def reca_features(img):
    ...
    return np.concatenate([p.ravel() for p in pp])  # (112*14 = 1568,)
...
# -----------------------------
# -----------------------------
# 8) Post‐training INT6 weight-only quantization
#     with TRUE bit packing + cached dequant fast path
# -----------------------------
print("8) Quantizing weights to INT6 (per-channel, symmetric) with true bit packing…")
_t = _tick()
...
# Make one model: INT6 only
qmodel6 = quantize_model_to_packed_intN(model, bits=6, cache_dequant=True).eval()
times["8_quantize_s"] = _tock(_t)
print("   ✅ Quantization + packing done for INT6.")
print(qmodel6)
print(f"   ⏱ Step 8 time: {times['8_quantize_s']:.3f} s\n")

# -----------------------------
# 9) Final evaluation on test set (INT6, CPU)
# -----------------------------
print("9) Evaluating INT6-packed model on CPU…")
Xte_cpu, yte_cpu = X_te_t.cpu(), y_te_t.cpu()

t6 = time.time()
with torch.no_grad():
    logits6 = qmodel6(Xte_cpu)   # first call unpacks+dequants and caches
    preds6  = logits6.argmax(1)
    acc6 = (preds6 == yte_cpu).float().mean().item() * 100
t6 = time.time() - t6

times["9_eval_cpu_s"] = t6
print(f"   Test Accuracy (INT6-packed): {acc6:.2f}%   ⏱ {t6:.3f}s\n")

# Use INT6 outputs for downstream plots
logits = logits6
preds  = preds6
...
plt.plot([0,1],[0,1],'k--')
plt.title("ROC Curves (INT6-packed)")
...
plt.title("Confusion Matrix (INT6-packed)")
plt.tight_layout()
plt.show()
