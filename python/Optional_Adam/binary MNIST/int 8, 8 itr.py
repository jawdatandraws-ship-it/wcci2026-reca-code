...
# Post-training INT8 weight-only quantization with true packing + cached fast path
...
# -----------------------------
# 8) Post‐training INT8 weight-only quantization
#     with TRUE bit packing + cached dequant fast path
# -----------------------------
print("8) Quantizing weights to INT8 (per-channel, symmetric) with true bit packing…")
_t = _tick()

def _signed_range(bits: int):
    assert 2 <= bits <= 8, "Supported bits: 2..8"
    qmin = -(1 << (bits - 1))
    qmax =  (1 << (bits - 1)) - 1
    return qmin, qmax
...
# Make one model: INT8 only ✅
qmodel8 = quantize_model_to_packed_intN(model, bits=8, cache_dequant=True).eval()
times["8_quantize_s"] = _tock(_t)
print("   ✅ Quantization + packing done for INT8.")
print(qmodel8)
print(f"   ⏱ Step 8 time: {times['8_quantize_s']:.3f} s\n")

# -----------------------------
# 9) Final evaluation on test set (INT8, CPU)
# -----------------------------
print("9) Evaluating INT8-packed model on CPU…")
Xte_cpu, yte_cpu = X_te_t.cpu(), y_te_t.cpu()

t8 = time.time()
with torch.no_grad():
    logits8 = qmodel8(Xte_cpu)   # first call unpacks+dequants and caches
    preds8  = logits8.argmax(1)
    acc8 = (preds8 == yte_cpu).float().mean().item() * 100
t8 = time.time() - t8

times["9_eval_cpu_s"] = t8
print(f"   Test Accuracy (INT8-packed): {acc8:.2f}%   ⏱ {t8:.3f}s\n")

# Use INT8 outputs for downstream plots
logits = logits8
preds  = preds8
...
plt.plot([0,1],[0,1],'k--')
plt.title("ROC Curves (INT8-packed)")
...
plt.title("Confusion Matrix (INT8-packed)")
plt.tight_layout()
plt.show()
