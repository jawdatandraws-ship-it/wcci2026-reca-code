# Full script with 8-bit weight quantization via PyTorch dynamic quantization
...
# -----------------------------
# (Re)define your ReCA feature pipeline...
# -----------------------------
def to_bitplanes(img):
    return np.array([(img.astype(np.uint8) >> i) & 1 for i in range(8)])
...
def grayscale_reform(evolved_layers):
    # recombine 8 bit-planes into one grayscale per iteration
    return [
        np.sum([(2 ** l) * layer[l] for l in range(8)], axis=0)
...
def reca_features(img):
    bps = to_bitplanes(img)                 # (8,28,28)
    evo = evolve_bitplanes(bps)             # list of 16 arrays (8,28,28)
...
# -----------------------------
def elastic_distort(image, alpha=30, sigma=5, rng=None):  
...
# -----------------------------
# 2) Extract ReCA features
# -----------------------------
print("2) Extracting ReCA features…")
_t = _tick()                                              
N = len(X_imgs)
...
# -----------------------------
# 4) Precompute elastic distortions
# -----------------------------
...
for s in range(2):
    ...
    for j in tqdm(range(55000), desc=f"   set {s+1}/2"):
        ...
        Xa[j] = reca_features(elastic_distort(orig_imgs[idx], rng=rng_aug))  
        ya[j] = orig_lbls[idx]
...
for epoch in trange(1, num_epochs+1, desc="   epochs"):
    ...
    if epoch in (11,14):
        ...
        train_loader = DataLoader(
            train_ds, batch_size=17000, shuffle=True, generator=g   
        )
...
# -----------------------------
# 10) ROC & confusion (same as before)
# -----------------------------
