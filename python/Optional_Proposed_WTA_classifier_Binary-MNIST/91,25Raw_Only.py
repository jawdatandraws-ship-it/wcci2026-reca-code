...
# NOTE: This version uses ROWS-ONLY CA (no column/vertical processing)
...
# ROWS-ONLY CA: left/right neighbors only
# -----------------------------
def to_bitplanes(img): return (img > 128).astype(np.uint8)[None,:,:]
...
def evolve_bitplanes(bitplanes, steps=8):
    # ROWS-ONLY: apply rule90 only along rows (left/right), no vertical processing
    out=[]; cur=bitplanes.copy()
    for _ in range(steps):
        x = np.array([rule90_step(bp, 0) for bp in cur])   # axis=0 => left/right
        out.append(x); cur=x
    return out

def grayscale_reform(e): return [layer[0] for layer in e]
...
# 2) Extract features
# -----------------------------
print("2) Extracting binary ReCA features (ROWS-ONLY CA)…"); _t=_tick()
...
# ---- deterministic (fixed) bias “jitter” ----
@torch.no_grad()
def deterministic_bias_offset(state):
    """Adds a fixed, seed-independent tiny offset pattern to break ties deterministically."""
    if state.get("use_bias", False):
        C = state["b"].numel()
        pattern = torch.arange(C, device=state["device"], dtype=torch.uint8) % 3  # 0,1,2,0,1,2,...
...
deterministic_bias_offset(model)  # fixed pattern (no randomness)
...
# Use a local generator with fixed seed for deterministic shuffling
...
t = time.time()
with torch.no_grad():
    logits = forward_logits(model, Xte)     # uint6 W & uint8 b cast to float inside forward
...
plt.plot([0,1],[0,1],'k--'); plt.title("ROC Curves (Final int6-trained weights) — ROWS-ONLY CA features")
...
plt.title("Confusion Matrix (Final int6-trained weights) — ROWS-ONLY CA features")
