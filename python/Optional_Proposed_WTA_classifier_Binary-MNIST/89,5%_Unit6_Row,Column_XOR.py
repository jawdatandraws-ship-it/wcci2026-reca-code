...
# One-pass uint6 perceptron-style training with tunable step ±x and int6
...
def evolve_bitplanes(bitplanes, steps=8):
    ...
    for _ in range(steps):
        row=np.array([rule90_step(bp,1) for bp in cur])
        col=np.array([rule90_step(bp,0) for bp in cur])
        x=np.bitwise_xor(row,col); out.append(x); cur=x
    return out
...
# 2) Extract features
# -----------------------------
print("2) Extracting binary ReCA features…"); _t=_tick()
...
plt.plot([0,1],[0,1],'k--'); plt.title("ROC Curves (Final int6-trained weights)")
...
plt.title("Confusion Matrix (Final int6-trained weights)")
